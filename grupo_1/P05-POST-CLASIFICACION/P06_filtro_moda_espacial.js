...
var param = { 
    code_region: 114,  //Region de Clasificacion 
    ...
    exclusion:{  // Indicar en la lista las clases y años a excluir en el filtro
      clases : [1,2],  //lista de clases a excluir en todos los años
    }
};
...
if(param.exclusion.clases.length>0){
   var clasifi = ee.List([])
      clasif_origin = clasif_origin.select(bandNames)
      param.exclusion.clases.forEach(function(clase){
        var clasif_code =clasif_origin.eq(clase).selfMask()
        clasifi = clasifi.add(clasif_origin.updateMask(clasif_code).selfMask())
      })
      ...
      print('Clases excluidos en el Filtro ', param.exclusion.clases);
}


classif_FS = classif_FS.select(bandNames).updateMask(regionRaster)
                          .set('code_region', param.code_region)
                          .set('version', param.version_output)
                          .set('descripcion', 'filtro espacial')

            
print('Result', classif_FS)
...
Map.addLayer(classif_FS.reproject('EPSG:4326', null, 4.7), vis3, 'class-SPATIAL FILTER'+param.month);
Map.addLayer(region,{},'region',false)

Export.image.toAsset({
    'image': classif_FS.byte(),
